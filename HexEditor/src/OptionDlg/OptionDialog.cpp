...
BOOL CALLBACK OptionDlg::run_dlgProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
  switch (Message) 
  {
    ...
    case WM_COMMAND : 
    {
      switch (LOWORD(wParam))
      {
        case IDOK:
          ...
          return TRUE;//::CallWindowProc(_hDefaultListProc, hwnd, Message, wParam, lParam);;
    ...
    case WM_NOTIFY:
    {
      ...
      if ((nmhdr.idFrom == IDC_TAB_PROP) && (nmhdr.code == (UINT)TCN_SELCHANGE)) {
        TabUpdate();
        return TRUE;
      }
      break;
...
void OptionDlg::TabUpdate(void)
{
  ...
  ::ShowWindow(::GetDlgItem(_hSelf, IDC_RADIO_MDIGITS),			bView);
  ::ShowWindow(::GetDlgItem(_hSelf, IDC_RADIO_MLIGATURES),			bView);
  ::ShowWindow(::GetDlgItem(_hSelf, IDC_RADIO_SDIGITS),			bView);
  ::ShowWindow(::GetDlgItem(_hSelf, IDC_RADIO_SLIGATURES),			bView);
...
void OptionDlg::SetParams(void)
{
  ...
  switch ( _pProp->hexProp.isBin )
  {
    ...
    case SHOW_MNEMONIC_DIGITS:
      ::SendDlgItemMessage(_hSelf, IDC_RADIO_MDIGITS, BM_SETCHECK, BST_CHECKED, 0);
      break;
    case SHOW_MNEMONIC_LIGATURES:
      ::SendDlgItemMessage(_hSelf, IDC_RADIO_MLIGATURES, BM_SETCHECK, BST_CHECKED, 0);
      break;
    case SHOW_SLANTED_DIGITS:
      ::SendDlgItemMessage(_hSelf, IDC_RADIO_SDIGITS, BM_SETCHECK, BST_CHECKED, 0);
      break;
    case SHOW_SLANTED_LIGATURES:
      ::SendDlgItemMessage(_hSelf, IDC_RADIO_SLIGATURES, BM_SETCHECK, BST_CHECKED, 0);
      break;
  }
...
BOOL OptionDlg::GetParams(void)
{
  ...
  if (bRet == TRUE) {
    ...
    /* get display form */
    if (::SendDlgItemMessage(_hSelf, IDC_RADIO_MDIGITS, BM_GETCHECK, 0, 0) == BST_CHECKED)
      _pProp->hexProp.isBin = SHOW_MNEMONIC_DIGITS;
    else if (::SendDlgItemMessage(_hSelf, IDC_RADIO_MLIGATURES, BM_GETCHECK, 0, 0) == BST_CHECKED)
      _pProp->hexProp.isBin = SHOW_MNEMONIC_LIGATURES;
    else if (::SendDlgItemMessage(_hSelf, IDC_RADIO_SDIGITS, BM_GETCHECK, 0, 0) == BST_CHECKED)
      _pProp->hexProp.isBin = SHOW_SLANTED_DIGITS;
    else if (::SendDlgItemMessage(_hSelf, IDC_RADIO_SLIGATURES, BM_GETCHECK, 0, 0) == BST_CHECKED)
      _pProp->hexProp.isBin = SHOW_SLANTED_LIGATURES;
    else if (::SendDlgItemMessage(_hSelf, IDC_RADIO_BIN, BM_GETCHECK, 0, 0) == BST_CHECKED)
      _pProp->hexProp.isBin = SHOW_BINARY;
    else
      _pProp->hexProp.isBin = SHOW_HEX;

    /* get autostart properties */
    ::GetWindowText(::GetDlgItem(_hSelf, IDC_EDIT_EXTLIST), _pProp->autoProp.szExtensions, sizeof(_pProp->autoProp.szExtensions)/sizeof(TCHAR));
    ::GetWindowText(::GetDlgItem(_hSelf, IDC_EDIT_PERCENT), _pProp->autoProp.szPercent, sizeof(_pProp->autoProp.szPercent) / sizeof(TCHAR));

    /* get font information */
    ::GetWindowText(::GetDlgItem(_hSelf, IDC_COMBO_FONTNAME), _pProp->fontProp.szFontName, sizeof(_pProp->fontProp.szFontName) / sizeof(TCHAR));
