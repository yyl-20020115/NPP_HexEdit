...
{
  HMODULE handle = ::GetModuleHandle(_T("HexEditor"));
  ...
  LPVOID resourceData = ::LockResource(rcData);

  if ( !resourceData )
    return false;

  data = static_cast<const char*>(resourceData);

  return true;
}

void InstallFontResource( int resourceName, const wchar_t* fontFilePath )
{
  DWORD size = 0;
  ...
  if ( !LoadFileInResource( resourceName, TTFONT_FILE, size, data ) )
  {
    systemMessage( _T("failed to load the resource") );
    return;
  }

  
  HANDLE fontFile = CreateFile( fontFilePath, GENERIC_WRITE, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL );
  if ( fontFile == INVALID_HANDLE_VALUE )
  {
    systemMessage( _T("failed to create file") );
    return;
  }
  if ( !WriteFile( fontFile, data, size, &bytesWritten, 0 ) )
  {
    systemMessage( _T("failed to write to file") );
    return;
  }
  ...
  {
    systemMessage( _T("failed to install font") );
    return;
  }
}
                       ...
                       LPVOID lpReserved )
{
  ...
  switch (reasonForCall)
  {
    case DLL_PROCESS_ATTACH:
    {
      ...
      InstallFontForMnemonicCharacters();
...
void loadSettings(void)
{
  ...
  if (::PathFileExists(configPath) == FALSE)
  {
    ...
    *_tcsrchr(configPath, '\\') = NULL;
    ...
    for (int i = ((int)vPaths.size()-1); i >= 0; i--)
    {
  ...
  *_tcsrchr(cmparePath, '\\') = NULL;
  ...
  ::GetPrivateProfileString(dlgEditor, fontname, _T("Courier New"), prop.fontProp.szFontName, 128, iniFilePath);
...
void openPropDlg(void)
{
  ...
  {
    // TODO: Here is where the font is updated when the options dialog is okayed.
    hexEdit1.UpdateFont();
    hexEdit2.UpdateFont();
    setHexMask();
    SystemUpdate();
...
BOOL LittleEndianChange(HWND hTarget, HWND hSource, LPINT offset, LPINT length)
{
  ...
  // TODO: the cause of "#266 HEX-Editor Plugin for Notepad++ v5.4 v0.9.5 (copy/paste bug)" is in this function.
...
eError replaceLittleToBig(HWND hTarget, HWND hSource, INT startSrc, INT startTgt, INT lengthOld, INT lengthNew)
{
  ...
  if (text != NULL)
  {
    if (hSource)
    {
      ...
      ::ScintillaGetText( hSource, text, startSrc, (LPARAM)startSrc + lengthNew );
    }
...
void DoCompare(void)
{
  ...
  if (cmpResult.hFile != INVALID_HANDLE_VALUE)
  {
    ...
    else
    {
      ...
      for (UINT i = (minLength / hexProp1.bits); i < (UINT)(maxLength / hexProp1.bits); i++)
      {
        ::WriteFile(cmpResult.hFile, &val, sizeof(val), &hasWritten, NULL);
      }
...
void Show(int aNumber)
{
  char buffer[64];
  MessageBoxA(NULL,_itoa(aNumber,buffer,10), "", 0 );
}
