        ...
        switch (((LPNMHDR)lParam)->code)
        {
          case LVN_GETDISPINFO:
          {
            ...
            if (lvItem->mask & LVIF_TEXT)
            {
              ...
              if ( _pCurProp != NULL )
              {
                // first column on the display
                if ( lvItem->iSubItem == 0 )
                  ReadAddressFromItem( wText, lvItem->iItem );
                // last column on the display
                else if ( lvItem->iSubItem == DUMP_FIELD )
                  ReadDumpFromItem( wText, lvItem->iItem );
                // middle columns
                else
                  ReadHexFromItem( wText, lvItem->iItem, lvItem->iSubItem );
              }
              ...
              ::WideCharToMultiByte( CP_ACP, 0, wText, -1, text, 129 );
              lvItem->pszText = text;
#endif
            }
            return TRUE;
    ...
    switch (Message)
    {
      ...
      case WM_LBUTTONDOWN:
      {
        ...
        else
        {
          ...
          if (0==(0x80 & ::GetKeyState(VK_SHIFT))) {
      ...
      case WM_CHAR:
      {
        ...
        if (_pCurProp->editType == HEX_EDIT_HEX) {
          if ( !OnCharItem(wParam, lParam) )
            return FALSE;
        } else {
          if ( !OnCharDump(wParam, lParam) )
            return FALSE;
        }
        break;
      }
...
void HexEdit::UpdateHeader(BOOL isFirstTime)
{
  ...
  if (_pCurProp->isVisible)
  {
    ...
    for (SHORT i = 0; i < _pCurProp->columns; i++)
    {
      ...
      ColSetupTermin.cx	= size.cx * (FACTOR * _pCurProp->bits + 1);
...
// Set the selection on a window, then get the selected text.  Use ScintillaGetText to fetch the text
// because SCI_GETSELTEXT converts NUL characters to spaces, which is not desirable.
inline UINT SetAndGetSelection( HWND window, INT startingPosition, INT endingPosition, char* buffer )
{
  ::SendMessage( window, SCI_SETSEL, startingPosition, (LPARAM)endingPosition );
  return ScintillaGetText( window, buffer, startingPosition, (LPARAM) endingPosition );
}
      ...
      case HEX_SEL_NORM:
      {
        ...
        if (clipboard.text != NULL) {
          if (LittleEndianChange(hSciTgt, _hParentHandle, &offset, &length) == TRUE) {
            clipboard.length = SetAndGetSelection( hSciTgt, posBeg - offset, posBeg - offset + clipboard.length, clipboard.text );
          }
        } else {
          ::MessageBox(_hParent, _T("Couldn't allocate memory."), _T("Explorer"), MB_OK | MB_ICONERROR);
        }
        break;
      }
      case HEX_SEL_BLOCK:
      {
        ...
        if (clipboard.text != NULL) {
          ...
          for (UINT i = 0; i < clipboard.items; i++)
          {
            ...
            if (LittleEndianChange(hSciTgt, _hParentHandle, &offset, &length) == TRUE) {
              SetAndGetSelection( hSciTgt, posBeg - offset, posBeg - offset + clipboard.stride, &clipboard.text[i*clipboard.stride] );
              posBeg += VIEW_ROW;
            }
          }
        } else {
...
void HexEdit::ReadAddressFromItem(WCHAR* wText, INT iItem)
{
  if ( _pCurProp == NULL )
    return;

  char formatString[32];
  sprintf( formatString, "%%0%i%c", _pCurProp->addWidth, getCLM() ? 'X' : 'x' );

  char text[129];
  sprintf( text, formatString, iItem * VIEW_ROW );

  ::MultiByteToWideChar( CP_ACP, 0, text, -1, wText, 129 );
}

void HexEdit::ReadHexFromItem(WCHAR* wText,INT iItem, INT iSubItem)
{
  char text[129];
  ...
  if (posBeg < _currLength)
  {
    if ((posBeg + _pCurProp->bits) < _currLength)
    {
      ...
      AddressConvert(text, _pCurProp->bits, wText );
    }
    else
    {
      ScintillaGetText(_hParentHandle, text, posBeg, _currLength);
      AddressConvert(text, _currLength - posBeg, wText);
      wmemset(&wText[(_currLength - posBeg)*FACTOR], 0x0020, SUBITEM_LENGTH);
...
void HexEdit::ReadDumpFromItem(WCHAR* wText,INT iItem)
{
  char text[129];
...
void HexEdit::AddressConvert(LPSTR bytes, INT length, WCHAR* result)
{
  CHAR asciiConversion[65];
  asciiConversion[0] = '\0';
  CHAR buffer[65];
  CHAR* bytesToConvert = buffer;

  // Reverse the string if the byte order is little-endian
  if ( _pCurProp->isLittle == TRUE )
  {
    for (INT i = 0; i < length; i++)
    {
      bytesToConvert[i] = bytes[length - i - 1];
    }
  }
  else
  {
    bytesToConvert = bytes;
  }
  ...
  case SHOW_BINARY:
      for (INT i = 0; i < length; i++)
      {
        strcat(asciiConversion, binMask[(UCHAR)bytesToConvert[i]]);
      }
    break;
    case SHOW_HEX:
      for (INT i = 0; i < length; i++)
      {
        strcat(asciiConversion, hexMask[(UCHAR)bytesToConvert[i]]);
      }
    break;
  ...
  case SHOW_SLANTED_DIGITS:
    for (INT i = 0; i < length; i++)
    {
      result[2*i] = 0xE000 + (((UCHAR) bytesToConvert[i] >> 4) & 0x0F);
      result[2*i + 1] = 0xE000 + ((UCHAR) bytesToConvert[i] & 0x0F);
    }
    result[2*length] = _T('\0');
    break;
  case SHOW_MNEMONIC_LIGATURES:
  case SHOW_SLANTED_LIGATURES:
    for (INT i = 0; i < length; i++)
    {
        result[i] = 0xE010 + (UCHAR) bytesToConvert[i];
    }
    result[length] = _T('\0');
    break;
  }

  if ( !SHOWING_MNEMONIC_CHARACTERS )
    ::MultiByteToWideChar( CP_ACP, 0, asciiConversion, -1, result, 129 );
}
    ...
    else
    {
        ...
        if (temp != nullptr) {
            ...
            for (UINT i = 1; i <= max; i++)
            {
                ...
                {
                    for (SHORT j = 1; j <= _pCurProp->bits && (_pCurProp->bits * i - j)<length; j++)
                    {
...
void HexEdit::TrackMenu(POINT pt)
{
  ...
  /* set decoding */
  ::AppendMenu(hSubMenu, MF_STRING | ((_pCurProp->isBin == SHOW_HEX)?MF_CHECKED:0), 11, _T("Hexadecimal"));
  ::AppendMenu(hSubMenu, MF_STRING | ((_pCurProp->isBin == SHOW_BINARY)?MF_CHECKED:0), 12, _T("Binary"));
#ifdef UNICODE
  ::AppendMenu(hSubMenu, MF_STRING | ((_pCurProp->isBin == SHOW_MNEMONIC_DIGITS)?MF_CHECKED:0), 13, _T("Alt hex digits 1"));
  ::AppendMenu(hSubMenu, MF_STRING | ((_pCurProp->isBin == SHOW_MNEMONIC_LIGATURES)?MF_CHECKED:0), 14, _T("Alt hex ligatures 1"));
  ::AppendMenu(hSubMenu, MF_STRING | ((_pCurProp->isBin == SHOW_SLANTED_DIGITS)?MF_CHECKED:0), 15, _T("Alt hex digits 2"));
  ::AppendMenu(hSubMenu, MF_STRING | ((_pCurProp->isBin == SHOW_SLANTED_LIGATURES)?MF_CHECKED:0), 16, _T("Alt hex ligatures 2"));
#endif
  ::AppendMenu(hSubMenu, MF_SEPARATOR, 0, _T("-----------------"));
  ...
  ::AppendMenu(hSubMenu, MF_STRING, 17, txtMenu);
               ...
               pt.x, pt.y, 0, _hListCtrl, NULL))
  {
    ...
    case 11:
    {
      _pCurProp->isBin	= SHOW_HEX;
    UpdateFont();
      break;
    }
  case 12:
    {
      _pCurProp->isBin	= SHOW_BINARY;
    UpdateFont();
      break;
    }
#ifdef UNICODE
    case 13:
    {
      _pCurProp->isBin = SHOW_MNEMONIC_DIGITS;
      UpdateFont();
      break;
    }
    case 14:
    {
      _pCurProp->isBin = SHOW_MNEMONIC_LIGATURES;
      UpdateFont();
      break;
    }
    case 15:
    {
      _pCurProp->isBin = SHOW_SLANTED_DIGITS;
      UpdateFont();
      break;
    }
    case 16:
    {
      _pCurProp->isBin = SHOW_SLANTED_LIGATURES;
      UpdateFont();
      break;
    }
#endif
...
void HexEdit::Delete(void)
{
  ...
  {
    count = abs( (int)_pCurProp->anchorPos - (int)_pCurProp->cursorPos );
    lines = abs( (int)_pCurProp->anchorItem - (int)_pCurProp->cursorItem );
  }
  else
  {
...
BOOL HexEdit::OnCharItem(WPARAM wParam, LPARAM lParam)
{
  ...
  // Treat the mnemonic character displays as if they were read-only.
  if ( SHOWING_MNEMONIC_CHARACTERS )
    return FALSE;

  if (_pCurProp->isBin == SHOW_HEX) {
...
void HexEdit::DrawDumpText(HDC hDc, DWORD item, INT subItem)
{
  ...
  if (_pCurProp->pCmpResult != NULL)
  {
    ...
    if (pos < CACHE_SIZE)
    {
      for (INT i = 0; i < _pCurProp->columns && pos<CACHE_SIZE; i++, pos++) {
...
void HexEdit::UpdateBookmarks(UINT firstAdd, INT length)
{
  ...
  for (UINT i = 0; i < _pCurProp->vBookmarks.size(); i++)
  {
    ...
    LONG_PTR	iItemTest	= _pCurProp->vBookmarks[i].iItem;
...
void HexEdit::CutBookmarkLines(void)
{
  ...
  if (clipboard.text != NULL)
  {
    ...
    for (UINT i = 0; i < clipboard.items; i++)
    {
      offset = (INT)_pCurProp->vBookmarks[i].iItem * VIEW_ROW - deleted;
      length = VIEW_ROW;
      if (LittleEndianChange(hSciTgt, _hParentHandle, &offset, &length) == TRUE)
      {
        SetAndGetSelection( hSciTgt, 0, length, &clipboard.text[i*VIEW_ROW] );
...
void HexEdit::CopyBookmarkLines(void)
{
  ...
  if (clipboard.text != NULL)
  {
    ...
    for (UINT i = 0; i < clipboard.items; i++)
    {
      offset = (INT)_pCurProp->vBookmarks[i].iItem * VIEW_ROW;
      length = VIEW_ROW;
      if (LittleEndianChange(hSciTgt, _hParentHandle, &offset, &length) == TRUE)
      {
        ::SendMessage(hSciTgt, SCI_SETSEL, 0, (LPARAM)length);
        ScintillaGetText(hSciTgt, &clipboard.text[i*VIEW_ROW], 0,  (LPARAM)length);
      }
    }
  }
  else
...
void HexEdit::PasteBookmarkLines(void)
{
  ...
  {
    /* copy into target scintilla */
    posBeg = (INT)_pCurProp->vBookmarks[i].iItem * VIEW_ROW;
...
void HexEdit::DeleteBookmarkLines(void)
{
  ...
  for (UINT i = 0; i < _pCurProp->vBookmarks.size(); i++)
  {
    posBeg = (UINT)_pCurProp->vBookmarks[i].iItem * VIEW_ROW - deleted;
    posEnd = posBeg + VIEW_ROW;
